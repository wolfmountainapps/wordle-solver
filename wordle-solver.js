 ...
 * Can find at https://screenrant.com/wordle-answers-updated-word-puzzle-guide/
...
/*
 * TBD: Could guess a word that isn't possible in order to eliminate letters to reduce the possible set. I do this sometimes when I play manually, and it can be quite helpful.
 * TBD: Could select a remaining possible word based on its similarity to other remaining possible words in order to eliminate more options if wrong. 
 */
...
const solveWordle = async() => {
    ...
    while (!solutionFound && row <= MAX_ATTEMPTS) {
        if (!solved(row)) {
            ...
            solutionFound = true;
            infoText.innerText += "\nSOLVED!";
            // TBD: Could show average score on the stats pop-up
        }
    }
}
/*
